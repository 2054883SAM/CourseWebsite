...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    console.log('Auth cookies at startup:', document.cookie.split(';')
      .map(c => c.trim())
    ...
    supabase.auth.getSession().then(({ data: { session }, error }) => {
      ...
      console.log('Initial auth session:', session ? 'Present' : 'None', 
  ...
  const signIn = async (email: string, password: string) => {
    try {
      ...
      if (data.session) {
        ...
        console.log('Cookies after sign in:', document.cookie.split(';')
          .map(c => c.trim())
          .filter(c => c.startsWith('sb-') || c.includes('supabase'))
        );
      }
